...
# Replace AlpacaBroker with IBBroker
ibkr_broker_code = """
class IBBroker:
    \"\"\"
    ...
    def buy(self, ticker: str, qty: int, limit_price: float, **kwargs) -> OrderResult:
        ...
        try:
            ...
            # Wait briefly to let IBKR acknowledge it
            self.ib.sleep(0.5)
...
# Replace AlpacaBroker class pattern
text = re.sub(
    r"class AlpacaBroker:.*?(?=\n# ─+ \n# BROKER FACTORY)",
    ibkr_broker_code,
    text,
    flags=re.DOTALL,
)

# Update get_broker factory
