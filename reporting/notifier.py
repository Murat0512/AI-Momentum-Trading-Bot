...
class DiscordNotifier:
    ...
    def send_eod_report(self, starting_balance: float, ending_balance: float):
        ...
        if not self.webhook_url:
            self.logger.warning(
                "?? Webhook URL not provided. EOD Report printed to terminal only."
            )
            return
            ...
            "embeds": [
                {
                    ...
                    "color": embed_color,
                    "fields": [
                        {"name": "Starting Balance", "value": f"", "inline": True},
                        {"name": "Ending Balance", "value": f"", "inline": True},
                        {"name": "Net P&L ($)", "value": f"**\**", "inline": False},
                        {
                            "name": "Net P&L (%)",
                            "value": f"**{pnl_pct:,.2f}%**",
                            "inline": False,
                    ...
                    "footer": {"text": "Interactive Brokers | Phase 5 Architecture"},
        ...
        try:
            ...
            if response.status_code == 204:
                ...
                self.logger.error(
                    f"? Failed to deliver Discord report. Status Code: {response.status_code}"
                )
